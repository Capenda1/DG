/**
 * Geração em massa de variantes Lona/Vinil (preço por m²).
 */
import {
...
export async function bulkCreateAreaVariantsForProduct(
  productId: string,
  productCode: string,
  opts: {
  ...
  const matrix = areaVariantMatrixForCode(productCode);
  if (!matrix?.length) {
    return {
      created: 0,
      skipped: 0,
      errors: [`O código «${productCode}» não tem matriz de lona/vinil.`],
    };
  }
...
function rowToVariantBody(row: AreaVariantRow): CreateAdminProductVariantBody {
  return {
    sku: row.sku,
    size: row.size,
    baseColor: null,
    ...
    currency: "AOA",
    active: true,
    metadata: row.metadata ?? { pricingKind: "AREA", areaUnit: "M" },
  };
}
